...
green_dist <- zone_income %>%
  ...
  left_join(select(green_pudo, LocationID, green_pu)) %>%
  mutate(dist_by_zone = proportion * green_pu) %>%
  ungroup() %>%
  group_by(category) %>%
  summarize(pu_income_dist = sum(dist_by_zone, na.rm = TRUE)) %>%
  mutate(taxi = "Green")

# ggplot(green_dist) +
#   geom_col(aes(x = category, y = pu_income_dist)) +
...
yellow_dist <- zone_income %>%
  select(LocationID, category, proportion) %>%
  left_join(select(yellow_pudo, LocationID, yellow_pu)) %>%
  mutate(dist_by_zone = proportion * yellow_pu) %>%
  ungroup() %>%
  group_by(category) %>%
  summarize(pu_income_dist = sum(dist_by_zone, na.rm = TRUE)) %>%
  mutate(taxi = "Yellow")
...
tnc_dist <- zone_income %>%
  select(LocationID, category, proportion) %>%
  left_join(select(tnc_pudo, LocationID, tnc_pu)) %>%
  mutate(dist_by_zone = proportion * tnc_pu) %>%
  ungroup() %>%
  group_by(category) %>%
  summarize(pu_income_dist = sum(dist_by_zone, na.rm = TRUE)) %>%
  mutate(taxi = "TNC")
...
pu_income_dist <- bind_rows(list(green_dist, yellow_dist, tnc_dist)) %>%
  mutate(taxi = factor(taxi)) %>%
  group_by(taxi) %>%
  mutate(proportion = pu_income_dist / sum(pu_income_dist))


# Plot

ggplot(pu_income_dist) +
...
ggsave(here("figures", "pu_income_dist.png"), width = 6.5, height = 5, dpi = 600)
...
ggplot(pu_income_dist) +
  ...
  scale_y_continuous(labels = comma) +
  scale_fill_manual(values = c("#9BD134", "#48C0DA", "#F7B731"))
ggsave(here("figures", "pu_relative_income_dist_1.png"), width = 6.5, height = 5, dpi = 600)
...
ggsave(here("figures", "pu_relative_income_dist_2.png"), width = 6.5, height = 5, dpi = 600)


# Collapsing income categories

pu_income_dist2 <- bind_rows(list(green_dist, yellow_dist, tnc_dist)) %>%
  ...
  summarize(pu_income_dist = sum(pu_income_dist)) %>%
  mutate(taxi = factor(taxi)) %>%
  group_by(taxi) %>%
  mutate(proportion = pu_income_dist / sum(pu_income_dist))
...
ggplot(pu_income_dist2) +
  ...
  scale_y_continuous(labels = comma) +
  scale_fill_manual(values = c("#9BD134", "#48C0DA", "#F7B731"))
ggsave(here("figures", "pu_income_dist_new_cats.png"), width = 6.5, height = 5, dpi = 600)
...
ggplot(pu_income_dist2) +
  ...
  scale_y_continuous(labels = comma) +
  scale_fill_manual(values = c("#9BD134", "#48C0DA", "#F7B731"))
ggsave(here("figures", "pu_relative_income_dist_1_new_cats.png"), width = 6.5, height = 5, dpi = 600)
...
ggplot(pu_income_dist2) +
  ...
  scale_y_continuous(labels = comma) +
  scale_color_manual(values = c("#9BD134", "#48C0DA", "#F7B731"))
ggsave(here("figures", "pu_relative_income_dist_2_new_cats.png"), width = 6.5, height = 5, dpi = 600)
